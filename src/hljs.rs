...
// Keeping the `ignoreIllegals` argument out of the default case, and since there is no optional arguments in
// Rust, this will have to be provided in a separate function (e.g. `highlight_ignore_illegals`), much like
// how `web_sys` does it for the browser APIs.
// For simplicity, only the highlighted HTML code is returned on success, and None on error.
